...
import { CheckBox, Close, Search } from "@/core-ui/public/icons";
import { convertToAsciiString, onlySpaces } from "@/utils/common";

import { ArrowMedium } from "../../public/icons/arrow/ArrowMedium";
...
>(
  (
    ...
    const handleSelection = (selectedIndex: number) => {
      const selectedItem = listData[selectedIndex] as OptionMulti;
      setListData(options || []);
    ...
    return (
      <Manager>
        ...
        >
            ...
            {({ ref }) => (
              <div
                ...
                className={classNames(
                  ...
                  visible && "border-primary-6 shadow-normal"
                )}
                ref={ref}
              >
                <div ref={setInputRef} {...rest}>
                  {select.length === 0 && (
                    <div className="text-base font-normal leading-6 text-ic-ink-4 ml-2">
                      {placeholder}
                    </div>
                  )}
                  {select.length === 1 && (
                    <div className="bg-ic-ink-6 flex items-center py-1 pl-3 pr-2 rounded-md">
          ...
          {error && (
            <p className="absolute mx-4 mt-1 text-red-6 text-sm font-normal leading-5">
              {error}
            </p>
          )}
          {visible && (
